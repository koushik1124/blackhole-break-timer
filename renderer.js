...
vec4 sampleDisk(vec3 pos, float r_eh, float uTime, float uDriftSpeed) {
  float rIn  = r_eh * 1.5; // ISCO outside shadow
  float rOut = r_eh * 4.5;
...
void main() {
  ...
  float r_eh = pow(progress, 2.0) * 0.45 + progress * 0.02;
  float r_ph = r_eh * 1.5;
  ...
  for (int i = 0; i < STEPS; i++) {
    ...
    vec3 gravityForce = -normalize(rayPos) * (1.4 * r_eh * r_eh / (r * r * r + 0.0001));
  ...
  float lensInfluenceRadius = pow(progress, 2.0) * 1.15;
...
void main() {
  float progress = clamp(uScale / 1.5, 0.0, 1.0);
  float eh = pow(progress, 2.0) * 0.45 + progress * 0.02;
  float scaledR = (aRadius * 0.7 + 0.3) * eh * 4.5;
